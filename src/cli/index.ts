    ...
    return [
        {
            ...
            summary: [
                "### Cache key issue found",
                "",
                "The **new cache layer** is a _useful_ performance win, but the *current key* omits `viewerRole` and can leak activity totals across permission scopes. The failing scenario is visible in the [authorization regression run](https://github.com/example/octoflow-api/actions/runs/184).",
                "",
                "> Reusing an owner summary for a viewer changes authorization, not just freshness.",
                "",
                "| Scope | Risk | Suggested fix |",
                "| --- | --- | --- |",
                "| `owner` -> `viewer` | Critical data exposure | Partition by role |",
                "| archived project | Stale dashboard state | Invalidate on archive/restore |",
                "",
                "Recommended follow-up:",
                "",
                "- [ ] Partition the cache by viewer role or permission scope.",
                "- [ ] Add coverage for **owner-to-viewer** cache reuse.",
                "- [x] Keep the cache miss path covered.",
                "",
                "Suggested key shape:",
                "",
                "```ts",
                "const cacheKey = `activity:${projectId}:${viewerRole}`;",
                "```",
                "",
   ...
   res.json({ projectId: project.id, summary });
            ...
            comments: [
                {
                    ...
                    severity: "critical",
                    body: [
                        "**Security:** this key only includes `projectId`, so a restricted viewer can receive a summary cached for an owner.",
                        "",
                        "Use a key that carries the authorization boundary:",
                        "",
                        "```ts",
                        "const cacheKey = `activity:${projectId}:${viewerRole}`;",
                        "```",
                        "",
                        "The [cache isolation checklist](https://docs.example.com/security/cache-isolation) has the expected cases.",
                    ].join("\n"),
                },
                {
                    path: "src/activity/cache.ts",
                    ...
                    severity: "warning",
                    body: [
                        "Consider invalidating this when projects are **archived** or **restored**.",
                        "",
                        "- `archiveProject` should delete activity cache entries.",
                        "- `restoreProject` should force the next request through `loadActivity`.",
                        "",
        ...
        },
        {
            ...
            summary: [
                ...
                "",
                "The validation should reject duplicate emails before sending invitations. The public contract in the [bulk invite API docs](https://docs.example.com/invitations/bulk) says one address can produce at most one invite.",
                "",
                "Proposed sequence:",
                "",
                "1. Normalize incoming addresses.",
                "2. Deduplicate the normalized list.",
                "3. Report duplicates as validation errors before calling `sendInvite`.",
                "",
                "```ts",
   ...
   return results;
            ...
            comments: [
                {
                    ...
                    severity: "suggestion",
                    body: [
                        "Normalize first, then deduplicate the list before the loop.",
                        "",
                        "Example that should return a single invite plus one validation error:",
                        "",
                        "```text",
        ...
        },
        {
            ...
            summary: [
                ...
                "",
                "| State | Selected plan | Submit button |",
                "| --- | --- | --- |",
                "| Plans loaded | `basic` | Enabled |",
                "| Plans empty | `undefined` | Should be disabled |",
                "",
                "> Keyboard navigation is still intact; this is a submit-state issue.",
            ].join("\n"),
            ...
            comments: [
                {
                    ...
                    severity: "warning",
                    body: [
                        "This still allows submit when `plans` is empty and `selectedPlan` is `undefined`.",
                        "",
                        "Disable until a concrete plan is selected:",
                        "",
                        "```tsx",
                        "const canSubmit = selectedPlan !== undefined && !isSaving;",
                        "```",
                    ].join("\n"),
                },
            ],
...
+        <MetricCard key={metric.id} metric={metric} />
        ...
        },
        {
            ...
            summary: [
                ...
                "",
                "- **Expected:** jobs stop after `MAX_ATTEMPTS`.",
                "- **Actual:** every failed job is re-enqueued.",
                "- **Reference:** [retry policy](https://docs.example.com/nimbus-worker/retries).",
            ].join("\n"),
            rawOutput:
...
+  }
            ...
            comments: [
                {
                    ...
                    severity: "warning",
                    body: [
                        "This removed the `MAX_ATTEMPTS` guard before enqueueing the next job.",
                        "",
        ...
        },
        {
            ...
            summary: [
                ...
                "Open questions:",
                "",
                "- Should disconnected subscribers be removed after `30s`?",
                "- Should metrics be dropped newest-first or oldest-first?",
                "- Where should the queue depth appear in the [operator dashboard](https://status.example.com/workers)?",
            ].join("\n"),
            ...
            comments: [
                {
                    ...
                    severity: "suggestion",
                    body: [
                        "This unbounded subscriber set depends on every caller invoking the returned cleanup.",
                        "",
                        "- [ ] Add idle timeouts.",
                        "- [ ] Cover disconnect cleanup in a lifecycle test.",
                        "- [ ] Emit a queue-depth metric when buffering starts.",
                    ].join("\n"),
                },
            ],
        },
